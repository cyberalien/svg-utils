...
const functionName = 'replaceIDs';

/**
 * Adds replaceIDs() function to assets
 */
export function addReplaceIDsFunctionAsset(
	imports: FactoryComponentImports,
	assets: GeneratedAssetFile[],
	options: Pick<ComponentFactoryOptions, 'rootPath' | 'helpersDirectory'>
) {
	const content = `const regex = /\\sid="(\\S+)"/g;
...
export function ${functionName}(body) {
	const ids = [];
	let match;
